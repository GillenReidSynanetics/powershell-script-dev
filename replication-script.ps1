...
try {
  ...
  if (Test-Path -Path (Join-Path -Path $scriptPath -ChildPath '.env')) {
    Copy-Item -Path (Join-Path -Path $scriptPath -ChildPath '.env') -Destination $backupFolder -ErrorAction Stop
  } else {
  ...
  if (Test-Path -Path (Join-Path -Path $scriptPath -ChildPath 'jwt')) {
    Copy-Item -Path (Join-Path -Path $scriptPath -ChildPath 'jwt') -Destination $backupFolder -Recurse -ErrorAction Stop
  } else {
    Write-Warning "'jwt' directory does not exist."
  }
